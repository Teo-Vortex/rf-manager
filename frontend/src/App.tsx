...
type Config = { host: string; port: number; username: string | null; password_configured: boolean; tls: boolean; client_id: string; receive_topic: string; command_topic: string; ha_enabled: boolean; ha_discovery_prefix: string; ha_base_topic: string };
...
const blankConfig: Config = { host: "host.docker.internal", port: 1883, username: "", password_configured: false, tls: false, client_id: "rf-manager", receive_topic: "tele/+/RESULT", command_topic: "cmnd/tasmota_A3F90F/", ha_enabled: true, ha_discovery_prefix: "homeassistant", ha_base_topic: "rfmanager" };
...
export function App() {
  ...
  async function clearEventHistory() {
    if (!confirm("Delete all saved RF activity? This cannot be undone.")) return;
    const response = await fetch("/api/events", { method: "DELETE" });
    if (!response.ok) { setNotice("Could not delete RF activity history."); return; }
    setEvents([]);
    setNotice("RF activity history deleted.");
  }
  ...
  return <div className="shell"><aside>
    ...
    {page === "live" && <><header><div><p className="eyebrow">REAL-TIME MONITOR</p><h1>Live RF activity</h1><p>Signals received from your Tasmota RF Bridge.</p></div><button className="secondary" onClick={() => setSettingsOpen(!settingsOpen)}>Configure MQTT</button></header>
      {settingsOpen && <section className="settings-card"><div><p className="eyebrow">CONNECTION</p><h2>MQTT broker</h2><p>On ZimaOS use <code>host.docker.internal</code>.</p></div><form onSubmit={saveSettings}>
        <label className="wide">Broker host<input value={config.host} onChange={e => setConfig({...config, host:e.target.value})} required /></label><label>Port<input type="number" value={config.port} onChange={e => setConfig({...config, port:Number(e.target.value)})} required /></label><label>Username<input value={config.username || ""} onChange={e => setConfig({...config, username:e.target.value})} /></label><label>Password<input type="password" value={password} placeholder={config.password_configured ? "Saved - leave blank to keep" : "Optional"} onChange={e => setPassword(e.target.value)} /></label><label>Client ID<input value={config.client_id} onChange={e => setConfig({...config, client_id:e.target.value})} /></label><label className="wide">Tasmota receive topic<input value={config.receive_topic} onChange={e => setConfig({...config, receive_topic:e.target.value})} /></label><label className="wide">Tasmota command topic<input value={config.command_topic} onChange={e => setConfig({...config, command_topic:e.target.value})} placeholder="cmnd/tasmota_A3F90F/" /></label><label className="check"><input type="checkbox" checked={config.tls} onChange={e => setConfig({...config, tls:e.target.checked})} /> Use TLS</label><label className="check wide"><input type="checkbox" checked={config.ha_enabled} onChange={e => setConfig({...config, ha_enabled:e.target.checked})} /> Enable Home Assistant MQTT Discovery</label>{config.ha_enabled && <><label>HA discovery prefix<input value={config.ha_discovery_prefix} onChange={e => setConfig({...config, ha_discovery_prefix:e.target.value})} /></label><label>RF Manager base topic<input value={config.ha_base_topic} onChange={e => setConfig({...config, ha_base_topic:e.target.value})} /></label></>}<div className="form-actions"><span>{notice}</span><button type="button" className="ghost" disabled={!status?.connected} onClick={async () => { const response = await fetch("/api/home-assistant/sync", {method:"POST"}); const body = await response.json(); setNotice(response.ok ? `Home Assistant: ${body.entities} entities synchronized.` : body.detail); }}>Refresh HA entities</button><button disabled={saving}>{saving ? "Saving..." : "Save & connect"}</button></div>
      </form></section>}
      <section className="toolbar"><input placeholder="Search code, device or bridge..." value={search} onChange={e => setSearch(e.target.value)} /><div><button className="ghost" onClick={() => setPaused(!paused)}>{paused ? "Resume" : "Pause"}</button><button className="ghost" onClick={clearEventHistory}>Delete history</button></div></section>
